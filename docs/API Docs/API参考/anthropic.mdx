...
<div className="tip">
  <p>OpenAI协议兼容Claude官方提供，属于官方的Beta功能，不排除未来官方废弃这种用法的可能，GPT API Proxy不保证这种用法的稳定性，建议尽量使用原生Anthropic SDK协议，Dify、Chatbox等大部分开源软件均支持原生协议。</p>
</div>
...
<div className="warning">
  <p>很多用户不懂如何区分协议，前面选择了OpenAI格式，然后后面域名处又写了/anthropic这种anthropic专用地址，造成请求404，这种纯属配置问题。使用claude模型一般软件比如ChatBox，都有专门的claude一项，不要在OpenAI的位置填claude模型的配置。</p>
</div>
